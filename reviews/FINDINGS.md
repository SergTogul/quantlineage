...
Confidence: HIGH
Status: **CLOSED** (2026-09-09). R0.9.1–R0.9.3 COMPLETE. Domain package has no FastAPI / `app.api` imports; HTTP `*Request` / `*Response` / RiskRun wire bodies live in `app.api.schemas`. `RiskEngine.calculate` / `HistoricalRiskEngine.calculate` return domain `RiskSummary`. `PortfolioService` is constructed in FastAPI lifespan via `build_portfolio_service()`, stored on `app.state`, and shared with `RiskRunWorker`; `get_portfolio_service(request)` reads `app.state` (legacy TestClient without lifespan uses the same factory as a fallback, not a module-global HTTP singleton). No DI container. **Named residual (not blocking close):** dual-use engine/service inputs (`AttributionRequest`, `RiskChangeAttributionRequest`, `WhatIfRequest`) remain in domain. Independent review pending. Evidence: `reviews/r0.9.1-split-transport-schemas-report.md`, `reviews/r0.9.2-typed-risk-results-report.md`, `reviews/r0.9.3-application-composition-report.md`.
...
Confidence: HIGH
Status: **IN PROGRESS** (2026-09-09). R0.4.1-A COMPLETE — `MarketSnapshot.equity` / `.rates_market` / `.vol` / `.fx` pinned as canonical typed views (`app.market.markets`); views wrap frozen MappingProxy fields; curve bp shifts use `shock_units.bps_to_decimal_rate`. R0.4.1-B COMPLETE — `VolMarket.surfaces` is `Mapping[str, VolSurface]`; `RateMarket.curves` exposes named `YieldCurve`; invalid present surface/curve payloads and non-6-letter FX spot keys fail closed. Empty `vol_surfaces` / `curves` / `fx_spots` remain valid. Flat dict storage retained (no nested Pydantic rewrite). R0.4.1-C COMPLETE — equity / equity-future / European-option Positions carry explicit `currency` (default `"USD"`); production Builtin/QuantLib rate lookup uses `terms.currency`; missing EUR (or other non-USD) rates on an explicit snapshot fail closed (`MissingMarketDataError rates[EUR]`), not silent USD 0.04. `MarketSnapshot.rates` constructor default remains `{"USD": 0.04}` when omitted; `rates={}` still injects nothing. R0.4.2-C/D — HTTP list + primary UI POST path are formal `ScenarioWire`; legacy StressScenario POST routes remain as **deprecated** back-compat only (RF-004 closed). R0.4.2-E — engine-facing stress / attribution / threat apply canonical `Scenario` (typed `FactorShock`); `StressScenario` is HTTP/wire adapter only (deprecated POSTs + DI libraries convert once via `to_canonical_scenario` / `stresses_to_scenarios`). **Do not CLOSE:** DEFAULT/THREAT libraries and persistence still *store* `StressScenario`; historical `MarketScenario` and reverse-stress factor-family solvers are not the same stored type. Evidence: `reviews/r0.4.1-c-explicit-currency-rate-report.md`, `reviews/r0.4.2-e-scenario-only-engine-report.md`.
