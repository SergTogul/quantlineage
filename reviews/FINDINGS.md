...
Confidence: HIGH
Status: **IN PROGRESS** (2026-09-09). R0.9.1 COMPLETE — obvious HTTP `*Request` / `*Response` / RiskRun wire bodies moved to `app.api.schemas` (`transport.py`). Domain package does not import FastAPI or `app.api`. Dual-use engine/service inputs (`AttributionRequest`, `RiskChangeAttributionRequest`, `WhatIfRequest`) remain in domain. R0.9.2 COMPLETE — `RiskEngine.calculate` / `HistoricalRiskEngine.calculate` return domain `RiskSummary` (`portfolio_id` from `portfolio.id`); callers consume the typed object, not `dict[str, float]`. **Do not CLOSE:** `PortfolioService` remains a process-wide orchestrator / no lifespan composition (R0.9.3). Evidence: `reviews/r0.9.1-split-transport-schemas-report.md`, `reviews/r0.9.2-typed-risk-results-report.md`.
