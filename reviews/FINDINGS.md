...
Status: **IN PROGRESS** (2026-09-09). R0.6.1–R0.6.6 APPROVE (architecture in place: `reviews/sdd-briefs/task-1-r0.6.3-review.md`, `task-2-r0.6.4-review.md`, `task-3-r0.6.6-review.md`, `task-4-r0.6.5-review.md`): stream shocked snapshots; one-pass scenario market (RF-006); QuantLib scalar-option reuse via live `SimpleQuote` handles; unique-shock valuation LRU bypass; factor contributions reuse joint P&L + O(N) Δ-Γ (not O(N×S×F) extra books); HEAVY `FULL_REVALUATION` is a RiskRun / Compose `worker` OS-process job (option B; no in-process QuantLib threads). Interactive default remains `DELTA_GAMMA`. Close-gate **KEEP OPEN** (`reviews/r0.6-rf007-close-gate-report.md`). Residual: R0.6.7 recorded PR-safe **10×50** (above 1×120) plus 1×120 identity. Close-matrix scores: N×S **PARTIAL** (10×50 + 1×120; N=100/1k **UNMET** in default PR); wall **MET** at 10×50; peak RSS **PARTIAL** (`peak_rss_kib` exists; process-lifetime `ru_maxrss`; QuantLib figure includes prior builtin); scenarios/sec **MET** at 10×50; builtin vs QuantLib **PARTIAL** (same N×S skip-or-run, but cash equity is `quantity * spot`, not N×S curve/instrument reconstruction); warm vs cold **MET** at 10×50; identity **MET** (`6602fa6906f2579f5c89af72a41ab274c07650234fff69387bc2202b5a40534f` 1×120, `a28cf4ee6199bf40da3f2598f4241fc85fa4adc4f86bccbbd97e4938047d7537` 10×50). No SLA. N=100/1k and S=750/1k remain **UNMET** in the default PR test (CLI `--acceptance-n` / `--acceptance-s` only). Option A intra-run scenario-block multiprocessing remains PARTIAL (may stay P1). Do not close.
...
Close-gate (2026-09-09): required direction 6 MET, 1 PARTIAL (option B job process, not intra-run chunks). Numerical identity **MET**. R0.6.7 PR-safe 10×50: wall **MET**, scenarios/sec **MET**, warm vs cold **MET**; N×S **PARTIAL**; peak RSS **PARTIAL** (process-lifetime `ru_maxrss`; QuantLib includes prior builtin); builtin vs QuantLib **PARTIAL** (cash equity `quantity * spot`, not reconstruction). N=100/1k / S=750/1k still **UNMET** in default PR. **KEEP OPEN.** RF-007 stays **IN PROGRESS**.
