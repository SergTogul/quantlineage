...
Requirements in the diff: European option reconstruction (not `quantity * spot`) at the same N×S with live spot/vol/rate/div; per-engine checksums; skip-or-run QuantLib; P&L gap at `rel=2e-3`; isolated RSS via a subprocess per impl; N=100×50 behind `RISKFORGE_NIGHTLY=1` with sibling job `full-reval-n100`; R0.6.1 / R0.6.7 cash-equity pins unchanged; RF-007 still IN PROGRESS / KEEP OPEN; no SLA floors; not in PR-FULL `needs:`.
...
- Nightly job `RISKFORGE_PRICING_ENGINE: builtin` is misleading (engines constructed directly).
