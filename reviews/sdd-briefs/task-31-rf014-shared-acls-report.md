...
- `QUANTLINEAGE_API_TOKENS` / optional `QUANTLINEAGE_API_PRINCIPAL`
...
- shared compose 443 / no host 8000 / no hardcoded `POSTGRES_PASSWORD=quantlineage`
...
RED: shared ACL tests failed at boot (`QUANTLINEAGE_API_TOKEN` required); compose file missing; RF-014 Status still IN PROGRESS. After implementation: **128 passed**, 1 known Starlette/httpx deprecation warning. Ruff: All checks passed.
...
Important items fixed: overlapping `QUANTLINEAGE_API_TOKEN` no longer remaps a `TOKENS` principal; `test_shared_null_owner_is_fail_closed`; shared frontend `VITE_API_BASE_URL=same-origin`.

## Next owner
- Labeled-runner SLA (RF-016) still **not MET** — needs a registered `self-hosted, quantlineage-sla` runner. Do not restore Milestone R0 COMPLETE until then.
