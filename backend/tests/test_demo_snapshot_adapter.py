...
def _usd_rate_book(*, reverse: bool=False) -> Portfolio:
    ...
    positions = [BondPosition(type='bond', id='bond-usd', issuer='UST', face_value=1000000, maturity_years=2.0, duration=1.9), SwapPosition(type='swap', id='swap-usd', notional=1000000, maturity_years=2.0, fixed_rate=0.04, duration=1.9)]
    if reverse:
        positions.reverse()
    return Portfolio(id='rates', name='rates', positions=positions)

@pytest.mark.parametrize('reverse', [False, True])
def test_position_dto_rejects_competing_usd_mark_fields(reverse: bool) -> None:
    """RF-001 Phase B: two USD instruments cannot embed competing rates on the DTO."""
...
def test_deprecated_position_provider_raises_marks_removed() -> None:
    with pytest.raises(SampleMarksRemovedError):
...
def test_mixed_usd_demo_uses_explicit_snapshot_successfully() -> None:
    ...
    valuations = BuiltinPricingEngine().value_portfolio(RATES_MACRO_PORTFOLIO, snapshot)
...
def _sample_desk_subset() -> Portfolio:
    ...
    return Portfolio(id='desk:Global Macro', name='Global Macro', positions=list(SAMPLE_PORTFOLIO.positions), firm=SAMPLE_PORTFOLIO.firm, desk='Global Macro', strategy=SAMPLE_PORTFOLIO.strategy)

def test_sample_desk_subset_raises_marks_removed_not_lww() -> None:
    """Adapter stay fail-closed; marks are gone so no LWW conflict path remains."""
    with pytest.raises(SampleMarksRemovedError) as exc_info:
        DemoSampleMarksSnapshotAdapter().snapshot(_sample_desk_subset())
    assert 'sample marks no longer live on Position' in str(exc_info.value)

def test_hierarchy_and_stress_do_not_rebuild_market_from_sample_desk_subset(monkeypatch) -> None:
...
def test_portfolio_service_threads_one_resolved_snapshot_to_dashboard_paths() -> None:
    ...
    service = PortfolioService(pricing, HistoricalRiskEngine(seed=1, observations=20))
    calls: list[str] = []
...
def test_compare_stress_and_var_legs_use_same_base_snapshot(monkeypatch) -> None:
    ...
    def tracking_calc(self, portfolio, pricing_engine, methodology=None, market=None):
        ...
        return orig_calc(self, portfolio, pricing_engine, methodology=methodology, market=market)
    ...
    service.compare_scenarios(SAMPLE_PORTFOLIO, hedge, [StressScenario(name='Crash', equity_shock=-0.2)])
...
def test_reverse_stress_multi_uses_root_market_snapshot_not_aggregate() -> None:
    ...
    result = service.reverse_stress_multi(SAMPLE_PORTFOLIO, 0.01, factors=['equity', 'vol'])
    assert calls == [SAMPLE_PORTFOLIO.id]
    assert aggregate_calls == []
    assert result.base_market_value != 0.0

def test_limit_drilldown_does_not_rebuild_market_from_sample_desk_subset(monkeypatch) -> None:
    ...
    monkeypatch.setattr(DemoSampleMarksSnapshotAdapter, 'snapshot', tracking)
    service = PortfolioService(BuiltinPricingEngine(), HistoricalRiskEngine(seed=1, observations=20))
    ref = HierarchyRef(level=HierarchyLevel.DESK, firm=SAMPLE_PORTFOLIO.firm, portfolio_id=SAMPLE_PORTFOLIO.id, desk=SAMPLE_PORTFOLIO.desk)
    report = service.limit_drilldown(portfolio=SAMPLE_PORTFOLIO, hierarchy=ref, breaches_only=False)
