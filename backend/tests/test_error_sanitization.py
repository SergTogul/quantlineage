...
# Deterministic fake exception text that must never appear in client payloads.
    ...
    try:
        with TestClient(app) as client:
            resp = client.post(
                "/risk/what-if",
                json={
                    "portfolio": _tiny_portfolio().model_dump(mode="json"),
                    "changes": [{"operation": "remove", "position_id": "eq-1"}],
                },
            )
    finally:
