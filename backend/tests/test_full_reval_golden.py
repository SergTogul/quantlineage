...
def test_historical_engine_full_reval_is_not_the_linear_path():
    ...
    shocked = historical_shocked_snapshots(market, dataset)
    expected = np.array(
        [pricing.value(pos, snap).market_value - base_pv for snap in shocked],
        dtype=float,
    )
    np.testing.assert_allclose(full_pnl, expected, atol=1e-12)

    linear = HistoricalRiskEngine(dataset=dataset).calculate(
        book, pricing, methodology=VaRMethodology.LINEAR, market=market
    )
    full = HistoricalRiskEngine(dataset=dataset).calculate(
        book, pricing, methodology=VaRMethodology.FULL_REVALUATION, market=market
    )
    assert full["methodology"] == "FULL_REVALUATION"
    assert abs(full["var_99"] - linear["var_99"]) > 1e-6
    losses = -full_pnl
