...
def _large_move_series() -> FactorObservationSeries:
    ...
    vol = np.array([0.40, 0.20, 0.10, 0.0, -0.05, 0.15, 0.30, 0.50, -0.10, 0.25])
    z = np.zeros(len(eq))
    return FactorObservationSeries(
        equity_returns=eq,
        vol_moves=vol,
...
def test_var_methodology_enum_values():
    assert {m.value for m in VaRMethodology} == {
        "LINEAR",
        "DELTA_GAMMA",
        "FULL_REVALUATION",
    }
...
def test_zero_shocks_yield_near_zero_var_all_methodologies():
    ...
    for meth in VaRMethodology:
        ...
        assert abs(r["market_value"]) > 0.0  # book still has value
...
def test_linear_omits_gamma_vs_delta_gamma():
    pricing = BuiltinPricingEngine()
    # Equity-only moves so the γ term is the sole LINEAR vs Δ-Γ difference.
    series = FactorObservationSeries(
        equity_returns=np.linspace(-0.12, 0.12, 25),
        vol_moves=np.zeros(25),
        rate_moves_bps=np.zeros(25),
        fx_returns=np.zeros(25),
    )
...
def test_var_report_exposes_methodology_and_reconciles_contributions():
    ...
    if pvar > 0:
        assert abs(sum(c.contribution_pct for c in report.contributions) - 100.0) < 1e-6
