...
def _book(n: int) -> dict[str, Any]:
    return Portfolio(
        id="workload-book",
        name="Workload Book",
        positions=[_equity(i) for i in range(n)],
...
def test_default_caps_match_documented_env_defaults() -> None:
    ...
    assert workload_mod.DEFAULT_MAX_REQUEST_BYTES == 1_048_576
    assert workload_mod.max_positions() == 500
    assert workload_mod.max_scenarios() == 50
    assert workload_mod.max_request_bytes() == 1_048_576
...
def test_invalid_or_non_positive_env_falls_back_to_defaults(
...
def test_payload_inspector_rejects_oversize_positions() -> None:
    with pytest.raises(workload_mod.WorkloadLimitExceeded) as exc:
        workload_mod.enforce_parsed_payload(
            {"id": "x", "name": "x", "positions": [{}] * 3},
            max_positions=2,
            max_scenarios=50,
        )
...
def test_payload_inspector_rejects_nested_portfolio_positions() -> None:
    nested = {
        "previous_portfolio": _book(1),
        "current_portfolio": _book(3),
    }
    with pytest.raises(workload_mod.WorkloadLimitExceeded) as exc:
...
def test_payload_inspector_rejects_scenarios_list_and_top_level_array() -> None:
    with pytest.raises(workload_mod.WorkloadLimitExceeded) as exc:
        workload_mod.enforce_parsed_payload(
            {"portfolio": _book(1), "scenarios": [_scenario(i) for i in range(3)]},
            max_positions=500,
            max_scenarios=2,
        )
...
def test_payload_inspector_allows_sample_shaped_payloads() -> None:
    workload_mod.enforce_parsed_payload(
        SAMPLE_PORTFOLIO.model_dump(mode="json"),
        max_positions=500,
        max_scenarios=50,
    )
    workload_mod.enforce_parsed_payload(
        {"portfolio": SAMPLE_PORTFOLIO.model_dump(mode="json"), "scenarios": [_scenario(0)]},
        max_positions=500,
        max_scenarios=50,
    )
...
def test_sample_portfolio_still_accepted_on_portfolio_bearing_routes() -> None:
    ...
    with TestClient(app) as client:
        ...
        custom = client.post(
            ...
            json={"portfolio": payload, "scenarios": [_scenario(0)]},
        )
...
def test_oversize_positions_rejected_with_envelope(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_oversize_positions_rejected_on_legacy_risk_route(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_oversize_scenarios_rejected_on_custom_stress(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_workload_http_exception_uses_existing_envelope_helper() -> None:
    ...
    exc = workload_mod.workload_http_exception(
        message="Request exceeds maximum positions",
        details={"field": "positions", "limit": 2, "actual": 3},
    )
    assert exc.status_code == 413
    assert exc.detail == error_payload(
...
def test_payload_inspector_rejects_what_if_adds_that_grow_book_past_cap() -> None:
    ...
    payload = {
        "portfolio": _book(1),
        "changes": [
            {"operation": "add", "position": _equity(10).model_dump(mode="json")},
            {"operation": "add", "position": _equity(11).model_dump(mode="json")},
            {"operation": "add", "position": _equity(12).model_dump(mode="json")},
        ],
    }
    with pytest.raises(workload_mod.WorkloadLimitExceeded) as exc:
...
def test_what_if_adds_over_position_cap_rejected_http(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("RISKFORGE_MAX_POSITIONS", "2")
    payload = {
...
def _asgi_post_chunked_typed_body(
    chunks: list[bytes],
    *,
    path: str = "/api/v1/market/snapshot",
) -> tuple[int, dict[str, Any], list[tuple[int, bool]]]:
    ...
    # Lifespan so typed-route deps that touch app.state are wired.
    with TestClient(app):
        asyncio.run(_invoke())
...
def test_full_app_chunked_typed_body_stops_before_fastapi_joins(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert body["details"]["limit"] == 64
    # Post-join / FastAPI-first implementations report the full offered size.
    ...
    # Tail must not be pulled off the ASGI stream.
    assert len(receive_log) == 1


def test_full_app_chunked_invalid_json_over_cap_is_413_not_422(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
