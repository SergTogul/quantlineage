...
swaption pricing require snapshot marks. Position DTOs carry economics only.
"""
...
def _local_fx_market(*, include_vol: bool = True) -> MarketSnapshot:
    """Former DTO-embedded marks for divergence checks (spot 1.50 / rates 10%/1%)."""
    payload: dict = {
        "id": "fx-local",
        "fx_spots": {"EURUSD": 1.50},
        "rates": {"USD": 0.10, "EUR": 0.01},
    }
    if include_vol:
        payload["fx_vols"] = {"EURUSD": 0.50}
    return MarketSnapshot(**payload)
...
def test_complete_snapshot_fx_forward_matches_cip_and_does_not_mutate_trade() -> None:
    ...
    local_pv = BuiltinPricingEngine().value(position, _local_fx_market(include_vol=False)).market_value
    assert valuation.market_value != local_pv
...
def test_complete_snapshot_fx_option_prices_without_mutating_trade() -> None:
    ...
    local_pv = BuiltinPricingEngine().value(position, _local_fx_market()).market_value
    assert valuation.market_value != local_pv
    assert position.model_dump(mode="json") == original


def test_fx_prices_from_explicit_local_style_snapshot() -> None:
    forward = _fx_forward()
    option = _fx_option()
    pricing = BuiltinPricingEngine()
    local = _local_fx_market()
    fwd_expected = _cip_pv(
        forward.notional_base, 1.50, forward.strike, forward.maturity_years, 0.10, 0.01
    )
    assert pricing.value(forward, local).market_value == pytest.approx(
        fwd_expected, rel=1e-12, abs=1e-9
    )
    assert pricing.value(option, local).market_value > 0.0
...
def test_aggregate_demo_snapshots_seed_required_fx_marks() -> None:
    ...
    assert seeded.fx_spots["EURUSD"] == 1.1
    ...
    assert resolved.fx_spots["EURUSD"] == 1.1
...
def _usd_rates(
    ...
    *,
    ...
    return MarketSnapshot(
        ...
        ir_future_quotes={"USD": quote} if quote is not None else {},
    )


@pytest.mark.parametrize("book_factory", [_bond, _swap], ids=["bond", "swap"])
@pytest.mark.parametrize("rates", [{}, {"EUR": 0.03}], ids=["empty-rates", "eur-only"])
def test_missing_bond_swap_rate_raises_when_market_is_supplied(book_factory, rates: dict[str, float]) -> None:
...
@pytest.mark.parametrize("quotes", [{}, {"EUR": 0.03}], ids=["empty-quotes", "eur-only"])
def test_missing_ir_future_quote_raises_when_market_is_supplied(quotes: dict[str, float]) -> None:
    market = MarketSnapshot(
        id="no-usd-quote",
        rates={"USD": 0.0425},
...
@pytest.mark.parametrize("book_factory", [_cap, _swaption], ids=["cap", "swaption"])
@pytest.mark.parametrize(
    "projection_rates", [{}, {"EUR": 0.03}], ids=["empty-proj", "eur-only"]
)
...
@pytest.mark.parametrize("book_factory", [_cap, _swaption], ids=["cap", "swaption"])
@pytest.mark.parametrize("ir_vols", [{}, {"EUR": 0.15}], ids=["empty-vols", "eur-only"])
...
def test_complete_snapshot_bond_matches_scalar_path_and_does_not_mutate_trade() -> None:
    ...
    assert valuation.market_value == pytest.approx(expected, rel=1e-12, abs=1e-9)
    other = BuiltinPricingEngine().value(position, MarketSnapshot(id="other", rates={"USD": 0.05}))
    assert valuation.market_value != other.market_value
    assert position.model_dump(mode="json") == original
...
def test_complete_snapshot_swap_prices_without_mutating_trade() -> None:
    ...
    valuation = BuiltinPricingEngine().value(position, market)
    expected = (0.041 - position.fixed_rate) * position.notional * position.duration
    assert valuation.market_value == pytest.approx(expected, rel=1e-12, abs=1e-9)
    other = BuiltinPricingEngine().value(position, MarketSnapshot(id="other", rates={"USD": 0.05}))
    assert valuation.market_value != other.market_value
    assert position.model_dump(mode="json") == original
...
def test_complete_snapshot_ir_future_prices_without_mutating_trade() -> None:
    ...
    valuation = BuiltinPricingEngine().value(position, market)
    expected = position.quantity * position.pv01 * (0.042 - 0.0425) * 10000.0
    assert valuation.market_value == pytest.approx(expected, rel=1e-12, abs=1e-9)
    other = BuiltinPricingEngine().value(
        position,
        MarketSnapshot(
            id="other",
            rates={"USD": 0.05},
...
def test_complete_snapshot_ir_options_price_without_mutating_trade() -> None:
    ...
    pricing = BuiltinPricingEngine()
    assert pricing.value(cap, market).market_value > 0.0
    assert pricing.value(swaption, market).market_value > 0.0
    richer = MarketSnapshot(
        id="ir-opt-rich",
        rates={"USD": 0.035},
        projection_rates={"USD": 0.04},
        ir_vols={"USD": 0.40},
    )
    assert pricing.value(cap, market).market_value != pricing.value(cap, richer).market_value
    assert pricing.value(swaption, market).market_value != pricing.value(swaption, richer).market_value
    assert cap.model_dump(mode="json") == original_cap
    assert swaption.model_dump(mode="json") == original_swaption


def test_rates_ir_price_from_explicit_local_style_snapshot() -> None:
    pricing = BuiltinPricingEngine()
    bond = _bond()
    t = _act365_fixed_years(bond.maturity_years)
    # Former DTO yield was 0.04 on auth fixtures after Phase A local paths.
    bond_market = _usd_rates(0.04)
    bond_expected = bond.face_value * bond.quantity * math.exp(-0.04 * t)
    assert pricing.value(bond, bond_market).market_value == pytest.approx(
        bond_expected, rel=1e-12, abs=1e-9
    )
    swap = _swap()
    swap_market = _usd_rates(0.04)
    swap_expected = (0.04 - swap.fixed_rate) * swap.notional * swap.duration
    assert pricing.value(swap, swap_market).market_value == pytest.approx(
        swap_expected, rel=1e-12, abs=1e-9
    )
    future = _ir_future()
    future_market = _usd_rates(0.04, projection=0.04, quote=0.04)
    future_expected = future.quantity * future.pv01 * (0.04 - 0.04) * 10000.0
    assert pricing.value(future, future_market).market_value == pytest.approx(
        future_expected, rel=1e-12, abs=1e-9
    )
    ir_opt = _usd_rates(0.04, projection=0.04, ir_vol=0.20)
    assert pricing.value(_cap(), ir_opt).market_value > 0.0
    assert pricing.value(_swaption(), ir_opt).market_value > 0.0
