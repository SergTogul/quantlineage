...
_eq_shock = st.floats(min_value=-0.4, max_value=-0.01, allow_nan=False, allow_infinity=False)

def _equity_book_and_market(n: int, data: st.DataObject) -> tuple[Portfolio, MarketSnapshot]:
    positions = []
    spots: dict[str, float] = {}
    for i in range(n):
        sym = f'S{i}'
        spots[sym] = data.draw(_price)
        positions.append(EquityPosition(type='equity', id=f'eq-{i}', symbol=sym, quantity=data.draw(_qty)))
    book = Portfolio(id='m9-prop', name='m9-prop', positions=positions)
    return book, equity_spots_market(spots)
...
def test_component_var_reconciles_random_equity_option_book(qty_eq, price, qty_opt, strike_mult, vol):
    ...
    book = Portfolio(id='cvar-prop', name='cvar-prop', positions=[EquityPosition(type='equity', id='eq', symbol='SPY', quantity=qty_eq), EuropeanOptionPosition(type='european_option', id='opt', symbol='SPY', quantity=qty_opt, strike=price * strike_mult, maturity_years=0.5, option_type='call')])
...
def test_stress_pnl_equals_sum_of_position_pnls(n, data, equity_shock):
    ...
    book, market = _equity_book_and_market(n, data)
    scenario = StressScenario(id='eq-shock', name='Equity shock', equity_shock=equity_shock, vol_shock=0.0, rates_shift_bps=0.0, fx_shock=0.0)
    results = stress.run(book, pricing, [scenario], market=market)
    assert len(results) == 1
    assert results[0].pnl == pytest.approx(sum(results[0].by_position.values()), abs=1e-09)
...
def test_empty_stress_scenario_list_returns_empty(n, data):
    ...
    if n:
        ...
        book = Portfolio(id='empty', name='empty', positions=[])
        market = MarketSnapshot(id='empty', rates={'USD': 0.04})
    assert stress.run(book, pricing, [], market=market) == []

@given(qty=_qty, price=_price, shock_a=_eq_shock, shock_b=_eq_shock)
@_PROP
def test_long_equity_stress_pnl_monotone_in_equity_shock(qty, price, shock_a, shock_b):
    ...
    book = Portfolio(id='long-eq', name='long-eq', positions=[EquityPosition(type='equity', id='eq', symbol='ABC', quantity=qty)])
    sa = StressScenario(id='a', name='a', equity_shock=shock_a)
    sb = StressScenario(id='b', name='b', equity_shock=shock_b)
    market = equity_spots_market({'ABC': price})
