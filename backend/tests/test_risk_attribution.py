...
def test_drivers_reconcile_to_total_change():
    ...
    current = previous.model_copy(
        update={
            "id": "shocked",
            "equity_spots": {k: v * 0.92 for k, v in previous.equity_spots.items()},
            "equity_vols": {k: v * 1.15 for k, v in previous.equity_vols.items()},
            "rates": {k: v + 0.0025 for k, v in previous.rates.items()},
            "fx_spots": {k: v * 0.98 for k, v in previous.fx_spots.items()},
        }
    )
    report = _engine().explain(
        RiskChangeAttributionRequest(
    ...
    assert abs(explained - report.total_change) <= max(
        _ABS_TOL, _REL_TOL * abs(report.total_change)
    )
    assert abs(report.residual) < _ABS_TOL
...
def test_new_and_closed_trades_attributed():
    ...
    shared = market.model_copy(
        update={
            "equity_spots": {
                **dict(market.equity_spots),
                **{k: market_curr.equity_spots[k] for k in market_curr.equity_spots},
            },
            "equity_vols": {
                **dict(market.equity_vols),
                **{k: market_curr.equity_vols.get(k, 0.25) for k in market_curr.equity_vols},
            },
        }
    )
    ...
    report = _engine().explain(
        RiskChangeAttributionRequest(
            ...
            previous_market=shared,
            current_market=shared,
        ),
        pricing,
    )
    ...
    # Closing the first equity should move risk (typically reduce for long equity).
...
def test_change_attribution_api():
    ...
    assert abs(payload["total_change"]) < 1e-4
