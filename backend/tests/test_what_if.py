...
def _hypo() -> EquityPosition:
    return EquityPosition(type='equity', id='eq-whatif', symbol='SPY', quantity=400.0, sector='ETF')
...
def test_service_what_if_returns_before_after_incremental_and_deltas():
    ...
    request = WhatIfRequest(portfolio=SAMPLE_PORTFOLIO, changes=[{'operation': 'add', 'position': _hypo()}], methodology=VaRMethodology.DELTA_GAMMA)
...
def test_what_if_remove_matches_apply_helper():
    svc = PortfolioService(BuiltinPricingEngine(), HistoricalRiskEngine(seed=7, observations=_OBS))
    request = WhatIfRequest(portfolio=SAMPLE_PORTFOLIO, changes=[{'operation': 'remove', 'position_id': 'opt-spy-put'}])
    report = svc.what_if(request)
    after_pf = apply_what_if_changes(SAMPLE_PORTFOLIO, [{'operation': 'remove', 'position_id': 'opt-spy-put'}])
    market = svc.market_snapshot(SAMPLE_PORTFOLIO)
    direct = svc.risk.calculate(after_pf, svc.pricing, methodology=VaRMethodology.DELTA_GAMMA, market=market)
