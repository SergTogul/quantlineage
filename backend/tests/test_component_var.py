...
def _mixed_series(n: int=100) -> FactorObservationSeries:
    ...
    return FactorObservationSeries(equity_returns=rng.normal(-0.001, 0.018, n), vol_moves=rng.normal(0.0, 0.07, n), rate_moves_bps=rng.normal(0.0, 3.5, n), fx_returns=rng.normal(0.0, 0.007, n))

def _option_equity_book() -> Portfolio:
    return Portfolio(id='cvar_book', name='Component VaR Book', positions=[EquityPosition(type='equity', id='eq-long', symbol='SPY', quantity=500, sector='ETF'), EuropeanOptionPosition(type='european_option', id='call-hedge', symbol='SPY', quantity=-200, strike=100.0, maturity_years=0.5, option_type='call'), EuropeanOptionPosition(type='european_option', id='put-long', symbol='SPY', quantity=300, strike=95.0, maturity_years=0.5, option_type='put')])
...
def test_single_position_component_equals_portfolio_parametric_var():
    pricing = BuiltinPricingEngine()
    book = Portfolio(id='solo', name='Solo', positions=[EquityPosition(type='equity', id='only', symbol='SPY', quantity=1000)])
    report = VaRAnalytics(dataset=ArrayHistoricalDataset(_mixed_series())).report(book, pricing, methodology=VaRMethodology.DELTA_GAMMA, market=equity_spot_market('SPY', 100.0))
    pvar = _parametric_var(report)
    assert len(report.contributions) == 1
    assert math.isclose(report.contributions[0].component_var, pvar, rel_tol=1e-08, abs_tol=1e-06)
    assert math.isclose(report.contributions[0].contribution_pct, 100.0, abs_tol=1e-09)
