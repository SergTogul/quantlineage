...
def test_equity_cash_delta_matches_analytic():
    pos = EquityPosition(type="equity", id="e", symbol="ABC", quantity=10, price=25.0)
...
def test_option_delta_gamma_vega_vs_analytic():
    ...
    assert any(m.factor == EquityVol("XYZ") for m in engine.calculate_position(pos, pricing, measures=("vega",)))
...
def test_key_rate_falls_back_to_parallel_when_missing():
    ...
    engine = SensitivityEngine(rate_bump_bps=1.0)
    dv01 = engine.calculate_position(pos, pricing, measures=("dv01",))[0].value
    kr_m = engine.calculate_position(pos, pricing, measures=("key_rate_dv01",))[0]
...
def test_sample_portfolio_emits_core_measures():
    ...
    results = engine.calculate(SAMPLE_PORTFOLIO, pricing)
...
def test_zero_positions_empty_sensitivities():
    ...
    assert SensitivityEngine().calculate(empty, pricing) == []


def test_future_measures_raise():
    pos = EquityPosition(type="equity", id="e", symbol="ABC", quantity=1, price=10)
    engine = SensitivityEngine()
    for name in sorted(FUTURE_MEASURES):
        with pytest.raises(NotImplementedError):
            engine.calculate_position(pos, pricing, measures=(name,))  # type: ignore[arg-type]


def test_unknown_measure_raises():
    pos = EquityPosition(type="equity", id="e", symbol="ABC", quantity=1, price=10)
    with pytest.raises(ValueError, match="unknown"):
        SensitivityEngine().calculate_position(pos, pricing, measures=("theta",))  # type: ignore[arg-type]
...
def test_portfolio_delta_aggregates_like_sum_of_positions():
    ...
    port = {
        ...
        for m in engine.calculate(portfolio, pricing, measures=("delta",))
        if m.name == "delta"
    }
    by_symbol: dict[str, float] = {}
    for pos in portfolio.positions:
        for m in engine.calculate_position(pos, pricing, measures=("delta",)):
