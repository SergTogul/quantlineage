...
def _var_body(*, price_token: str) -> bytes:
    payload = {
        "id": "finite-scalar-book",
        "name": "Finite Scalar Book",
        "positions": [
            {
                "type": "equity",
                "id": "eq-1",
    ...
    return raw.replace('"price": 100.0', f'"price": {price_token}').encode()
...
def test_var_endpoint_rejects_non_finite_json_price(token: str) -> None:
    ...
    with TestClient(app, raise_server_exceptions=False) as client:
        response = client.post(
            "/api/v1/risk/var",
            content=_var_body(price_token=token),
...
def test_formal_custom_rejects_non_finite_json_amount(path: str, token: str) -> None:
    ...
    with TestClient(app, raise_server_exceptions=False) as client:
        response = client.post(
            path,
            content=_formal_custom_body(amount_token=token),
            headers={"Content-Type": "application/json"},
        )
    _assert_json_safe_validation_422(response)
...
def test_formal_custom_rejects_non_finite_json_max_loss_pct(path: str, token: str) -> None:
    ...
    with TestClient(app, raise_server_exceptions=False) as client:
        response = client.post(
            path,
            content=_formal_custom_body(max_loss_token=token),
            headers={"Content-Type": "application/json"},
        )
    _assert_json_safe_validation_422(response)
