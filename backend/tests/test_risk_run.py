...
def test_risk_run_status_values():
    assert {s.value for s in RiskRunStatus} == {'QUEUED', 'RUNNING', 'COMPLETED', 'FAILED'}
...
def test_duration_computed_from_timestamps():
    ...
    run = RiskRun(id='run-1', portfolio_id='p-1', status=RiskRunStatus.COMPLETED, started_at=start, completed_at=end, methodology=VaRMethodology.DELTA_GAMMA, pricing_engine_version='builtin-1.0', scenario_set=['scn-a', 'scn-b'], result_refs=[RiskResultRef(result_type='summary', result_id=7)])
    assert run.duration == pytest.approx(45.5)

@pytest.mark.parametrize('kwargs,match', [({'id': ' ', 'portfolio_id': 'p'}, 'id must be non-empty'), ({'id': 'r', 'portfolio_id': ' '}, 'portfolio_id must be non-empty'), ({'id': 'r', 'portfolio_id': 'p', 'status': RiskRunStatus.QUEUED, 'started_at': _ts()}, 'QUEUED runs must not have started_at'), ({'id': 'r', 'portfolio_id': 'p', 'status': RiskRunStatus.RUNNING}, 'RUNNING runs require started_at'), ({'id': 'r', 'portfolio_id': 'p', 'status': RiskRunStatus.COMPLETED, 'started_at': _ts()}, 'COMPLETED runs require started_at and completed_at'), ({'id': 'r', 'portfolio_id': 'p', 'status': RiskRunStatus.FAILED, 'started_at': _ts(), 'completed_at': _ts(12, 1)}, 'FAILED runs require a non-empty error'), ({'id': 'r', 'portfolio_id': 'p', 'status': RiskRunStatus.COMPLETED, 'started_at': _ts(12, 2), 'completed_at': _ts(12, 1)}, 'completed_at must be >= started_at'), ({'id': 'r', 'portfolio_id': 'p', 'scenario_set': ['ok', '']}, 'scenario_set entries must be non-empty'), ({'id': 'r', 'portfolio_id': 'p', 'pricing_engine_version': '  '}, 'pricing_engine_version must be non-empty')])
def test_risk_run_validation_rejects(kwargs, match):
    with pytest.raises(ValidationError, match=match):
        RiskRun(**kwargs)

def test_failed_run_requires_error_ok():
    run = RiskRun(id='run-f', portfolio_id='p-1', status=RiskRunStatus.FAILED, started_at=_ts(), completed_at=_ts(12, 1), error='pricing timeout')
    assert run.error == 'pricing timeout'
    assert run.duration == pytest.approx(60.0)

def test_orm_round_trip_mapping_aliases():
    run = RiskRun(id='run-m', portfolio_id='p-1', market_snapshot_id='snap-1', status=RiskRunStatus.COMPLETED, started_at=_ts(), completed_at=_ts(12, 1), pricing_engine_version='quantlib-1.36', methodology=VaRMethodology.FULL_REVALUATION, scenario_set=['eq-crash'], run_type='stress', request={'horizon': 1}, result_refs=[])
...
def test_apply_risk_run_to_row_updates_error_alias():
    run = RiskRun(id='run-e', portfolio_id='p-1', status=RiskRunStatus.FAILED, started_at=_ts(), completed_at=_ts(12, 1), error='boom')
    row = risk_run_to_row(run)
...
def test_risk_run_repository_domain_round_trip(session_factory):
    ...
    queued = RiskRun(id='run-1', portfolio_id='p-run', market_snapshot_id='snap-run', pricing_engine_version='builtin-1.0', methodology=VaRMethodology.DELTA_GAMMA, scenario_set=['base'], run_type='summary', request={'confidence': 0.99})
