...
def _new_equity(position_id: str='eq-hypo', quantity: float=500.0) -> EquityPosition:
    return EquityPosition(type='equity', id=position_id, symbol='SPY', quantity=quantity, sector='ETF')
...
def test_apply_rejects_invalid_ops():
    ...
    with pytest.raises(ValueError, match='already exists'):
        apply_what_if_changes(SAMPLE_PORTFOLIO, [{'operation': 'add', 'position': _new_equity('eq-spy')}])
...
def test_incremental_var_equals_after_minus_before():
    ...
    result = incremental_var(SAMPLE_PORTFOLIO, pricing, changes=[{'operation': 'add', 'position': trade}], risk_engine=engine, methodology=VaRMethodology.DELTA_GAMMA, market=market)
...
def test_removing_all_risk_positions_reduces_var():
    ...
    pricing = BuiltinPricingEngine()
    engine = _engine()
    result = incremental_var(SAMPLE_PORTFOLIO, pricing, changes=[{'operation': 'remove', 'position_id': 'eq-nvda'}], risk_engine=engine, market=demo_market_snapshot(SAMPLE_PORTFOLIO))
    assert result.incremental.var_99 <= 0.0 + _TOL

def test_empty_changes_zero_incremental():
    pricing = BuiltinPricingEngine()
    engine = _engine()
    result = incremental_var(SAMPLE_PORTFOLIO, pricing, changes=[], risk_engine=engine, market=demo_market_snapshot(SAMPLE_PORTFOLIO))
...
def test_zero_portfolio_add_trade_incremental_equals_standalone_var():
    ...
    market = equity_spot_market(trade.symbol, 100.0)
