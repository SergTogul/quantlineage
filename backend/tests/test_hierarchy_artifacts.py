...
def _complete_artifacts() -> dict[str, TradeCalculationArtifact]:
    return {'eq-a': _artifact('eq-a', pv=100.0, delta=10.0, gamma=0.5, vega=2.0, dv01=0.1, fx_delta=3.0, stress_pnl={'eq_crash': -10.0, 'rate_up': -2.0}), 'eq-b': _artifact('eq-b', pv=40.0, delta=-4.0, gamma=0.25, vega=1.5, dv01=-0.02, fx_delta=1.0, stress_pnl={'eq_crash': -5.0, 'fx_shock': 1.5})}
...
def test_parent_additive_metrics_equal_sum_of_artifacts_without_pricing():
    ...
    trades = [n for n in root.children[0].children[0].children[0].children[0].children if n.level == 'trade']
...
def test_risk_at_sums_artifacts_without_pricing():
    ...
    artifacts = _complete_artifacts()
    expected = artifacts['eq-a'].add(artifacts['eq-b'], trade_id='node')
    ref = HierarchyRef(level=HierarchyLevel.BOOK, firm='Acme Capital', portfolio_id='two-trade', desk='Equity Desk', strategy='Momentum', book='Cash')
    node = _engine().risk_at(pf, _ForbiddenPricing(), ref, market=_two_trade_market(), artifacts=artifacts)
