...
def _multi_book_market() -> MarketSnapshot:
    # Spots chosen so Σ qty*spot == 3500 (pins additive MV assertion).
    return equity_spots_market({"AAA": 10.0, "BBB": 20.0, "CCC": 40.0, "DDD": 50.0})

def _multi_book_portfolio() -> Portfolio:
    """Two desks, two books under one desk, inherited placement on one trade."""
    return Portfolio(id='multi-desk', name='Multi Desk Book', firm='Acme Capital', desk='Default Desk', strategy='Default Strat', positions=[EquityPosition(type='equity', id='eq-a', symbol='AAA', quantity=100, book='Cash A', desk='Rates Desk', strategy='Carry'), EquityPosition(type='equity', id='eq-b', symbol='BBB', quantity=50, book='Cash B', desk='Equity Desk', strategy='Momentum'), EquityPosition(type='equity', id='eq-c', symbol='CCC', quantity=25, book='Cash B'), EquityPosition(type='equity', id='eq-d', symbol='DDD', quantity=10, book='Cash C', desk='Equity Desk', strategy='Momentum')])
...
def test_hierarchy_node_id_matches_portfolio_at_scheme():
    ...
    cases = [(HierarchyRef(level=HierarchyLevel.FIRM, firm='Acme Capital'), 'firm:Acme Capital'), (HierarchyRef(level=HierarchyLevel.PORTFOLIO, firm='Acme Capital', portfolio_id='multi-desk'), 'portfolio:multi-desk'), (HierarchyRef(level=HierarchyLevel.DESK, firm='Acme Capital', portfolio_id='multi-desk', desk='Rates Desk'), 'desk:Rates Desk'), (HierarchyRef(level=HierarchyLevel.STRATEGY, firm='Acme Capital', portfolio_id='multi-desk', desk='Rates Desk', strategy='Carry'), 'strategy:Rates Desk/Carry'), (HierarchyRef(level=HierarchyLevel.BOOK, firm='Acme Capital', portfolio_id='multi-desk', desk='Equity Desk', strategy='Momentum', book='Cash C'), 'book:Equity Desk/Momentum/Cash C'), (HierarchyRef(level=HierarchyLevel.TRADE, firm='Acme Capital', portfolio_id='multi-desk', desk='Rates Desk', strategy='Carry', book='Cash A', trade_id='eq-a'), 'trade:Rates Desk/Carry/Cash A/eq-a')]
    for ref, expected in cases:
        assert hierarchy_node_id(ref.level, firm=pf.firm, portfolio_id=pf.id, desk=ref.desk, strategy=ref.strategy, book=ref.book, trade_id=ref.trade_id) == expected
...
def test_portfolio_id_equal_to_trade_id_does_not_collide():
    pricing = BuiltinPricingEngine()
    pf = Portfolio(id='eq-a', name='Named Like A Trade', firm='Acme', positions=[EquityPosition(type='equity', id='eq-a', symbol='AAA', quantity=1), EquityPosition(type='equity', id='eq-b', symbol='BBB', quantity=1)])
    root = _engine().build(pf, pricing, market=_multi_book_market())
...
def test_empty_and_literal_trade_ids_under_different_books_do_not_collide():
    pricing = BuiltinPricingEngine()
    pf = Portfolio(id='books', name='Empty vs literal trade', firm='Acme', desk='D', strategy='S', positions=[EquityPosition(type='equity', id='', symbol='AAA', quantity=1, book='Cash A'), EquityPosition(type='equity', id='trade', symbol='BBB', quantity=1, book='Cash B')])
    root = _engine().build(pf, pricing, market=_multi_book_market())
    ids = _assert_unique_nonempty_ids(root)
...
def test_trade_id_matching_desk_prefix_does_not_collide():
    pricing = BuiltinPricingEngine()
    pf = Portfolio(id='desk-collision', name='Trade named like a desk', firm='Acme', desk='Rates Desk', strategy='Carry', positions=[EquityPosition(type='equity', id='desk:Rates Desk', symbol='AAA', quantity=1, book='Cash A')])
    root = _engine().build(pf, pricing, market=_multi_book_market())
    ids = _assert_unique_nonempty_ids(root)
...
def test_empty_and_literal_book_labels_do_not_collide():
    pricing = BuiltinPricingEngine()
    pf = Portfolio(id='book-labels', name='Empty vs literal book', firm='Acme', desk='D', strategy='S', positions=[EquityPosition(type='equity', id='eq-empty-book', symbol='AAA', quantity=1, book=''), EquityPosition(type='equity', id='eq-book', symbol='BBB', quantity=1, book='book')])
    root = _engine().build(pf, pricing, market=_multi_book_market())
    ids = _assert_unique_nonempty_ids(root)
...
def test_empty_portfolio_id_is_namespaced_and_nonempty():
    pricing = BuiltinPricingEngine()
    pf = Portfolio(id='', name='Empty book id', firm='Acme', positions=[EquityPosition(type='equity', id='eq-z', symbol='AAA', quantity=1)])
    root = _engine().build(pf, pricing, market=_multi_book_market())
    ids = _assert_unique_nonempty_ids(root)
    assert 'portfolio:' in ids

def test_slash_in_desk_and_strategy_labels_do_not_collide():
    assert hierarchy_node_id('strategy', firm='Acme', portfolio_id='p', desk='Rates/Extra', strategy='Carry') != hierarchy_node_id('strategy', firm='Acme', portfolio_id='p', desk='Rates', strategy='Extra/Carry')
