...
def _bond_10y() -> BondPosition:
    return BondPosition(type='bond', id='b10', issuer='UST', face_value=1000000.0, quantity=1.0, maturity_years=10.0, duration=8.0)

def _swap_5y() -> SwapPosition:
    return SwapPosition(type='swap', id='s5', notional=5000000.0, maturity_years=5.0, fixed_rate=0.04, pay_fixed=True, duration=4.3)
...
def test_bond_falls_back_to_continuous_act365_without_curves():
    ...
    assert pv == pytest.approx(1000000.0 * math.exp(-0.05 * 10.0), rel=1e-12)
...
def test_bond_uses_continuous_df_when_usd_curves_attached():
    ...
    pv = builtin.value(pos, market).market_value
    assert pv == pytest.approx(1000000.0 * math.exp(-0.04 * 10.0), rel=1e-12)
...
def test_tenor_bump_changes_bond_pv_off_pillar_does_not():
    ...
    assert abs(base - parallel) >= abs(base - up_10) - 1e-06

def test_tenor_bump_changes_swap_pv_differently_by_pillar():
    pos = _swap_5y()
    market = attach_standard_usd_curves(MarketSnapshot(id='c', rates={'USD': 0.04}), ois_rate=0.04, sofr_rate=0.041)
    base = builtin.value(pos, market).market_value
...
def test_ir_future_uses_projection_curve_tenor():
    pos = InterestRateFuturePosition(type='ir_future', id='ed', quantity=10, pv01=25.0, maturity_years=1.0)
    market = attach_standard_usd_curves(MarketSnapshot(id='c', rates={'USD': 0.04}, ir_future_quotes={'USD': 0.041}), ois_rate=0.04, sofr_rate=0.041)
    base = builtin.value(pos, market).market_value
    assert base == pytest.approx(0.0, abs=1e-09)
...
def test_bootstrapped_curve_feeds_builtin_bond_and_swap_pricing():
    market = attach_bootstrapped_curve(MarketSnapshot(id='boot', rates={'USD': 0.01}), currency='USD', curve_type='discount', name='USD_BOOT', instruments=[CurveBootstrapInstrument(kind='deposit', tenor='6M', rate=0.04), CurveBootstrapInstrument(kind='zero', tenor='1Y', rate=0.041), CurveBootstrapInstrument(kind='zero', tenor='2Y', rate=0.042), CurveBootstrapInstrument(kind='zero', tenor='5Y', rate=0.045)])
    bond = BondPosition(type='bond', id='b2', issuer='UST', face_value=1000000.0, quantity=1.0, maturity_years=2.0, duration=1.9)
    swap = SwapPosition(type='swap', id='s5_boot', notional=5000000.0, maturity_years=5.0, fixed_rate=0.04, pay_fixed=True, duration=4.3)
    assert builtin.value(bond, market).market_value == pytest.approx(1000000.0 * math.exp(-0.042 * 2.0))
    assert builtin.value(swap, market).market_value == pytest.approx((0.045 - 0.04) * 5000000.0 * 4.3)
...
def test_ql_tenor_bump_changes_bond_pv(ql_engine):
    ...
    assert up_10 < base
    assert abs(base - up_10) > 100.0
    assert up_2 == pytest.approx(base, rel=1e-08)
