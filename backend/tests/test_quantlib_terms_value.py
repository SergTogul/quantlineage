"""R0.2 leftover — QuantLib value() prices from InstrumentTerms + snapshot.
...
def test_value_uses_terms_quantity_not_dto_when_terms_diverge(engine, monkeypatch):
    ...
    if hasattr(quantlib_mod, "terms_from_position"):
        monkeypatch.setattr(quantlib_mod, "terms_from_position", doubled_quantity)
