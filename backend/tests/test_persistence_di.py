...
def test_depends_repos_available_without_database_url(clear_db_url):
    ...
    def _probe(
        market: MarketSnapshotRepository = Depends(get_market_snapshot_repository),
        scenarios: ScenarioDefinitionRepository = Depends(get_scenario_definition_repository),
        limits: LimitDefinitionRepository = Depends(get_limit_definition_repository),
        snap=Depends(get_default_market_snapshot),
    ):
        return {
            "snapshot_id": snap.id,
            "scenario_count": len(scenarios.list_all()),
            "limit_count": len(limits.list_for_portfolio(None)),
            "market_has_seed": market.get(DEFAULT_MARKET_SNAPSHOT_ID) is not None,
        }
...
def test_default_risk_runs_use_memory_without_database_url(clear_db_url, tiny_portfolio):
    ...
    with TestClient(app) as client:
        ...
        # Memory path: no session_factory on app.state
...
def tiny_portfolio() -> Portfolio:
    return Portfolio(
        ...
        positions=[
            EquityPosition(
                type="equity", id="eq-di-1", symbol="AAPL", quantity=10, price=100.0
            )
        ],
    )
...
def test_database_url_wires_sqlalchemy_portfolio_and_risk_runs(monkeypatch, tmp_path, tiny_portfolio):
    ...
    # Re-import wiring after env is set so lifespan reads the URL.
    # App module is already loaded; lifespan reads env at request time.
    from app.main import app

    with TestClient(app) as client:
        assert client.app.state.persistence_enabled is True
        assert client.app.state.session_factory is not None
        # SQLAlchemy path: session-scoped Depends; no long-lived memory repos.
        ...
        # GET /portfolio serves seeded sample from SQLAlchemy.
        resp = client.get("/portfolio")
        assert resp.status_code == 200
        body = resp.json()
        assert body["id"] == SAMPLE_PORTFOLIO.id
        ...
        # Risk runs persist via SQLAlchemy (same pattern as M5.4 worker test).
        created = client.post(
            "/risk/runs",
            json={
                "portfolio": tiny_portfolio.model_dump(mode="json"),
    ...
    def _probe(
        ...
        return {
            ...
            "repo_types": [
                ...
                type(scenarios).__name__,
                type(limits).__name__,
            ],
        }
...
def test_sqlalchemy_portfolio_reflects_db_updates(monkeypatch, tmp_path):
    ...
    with TestClient(app) as client:
        factory = client.app.state.session_factory
        updated = SAMPLE_PORTFOLIO.model_copy(
            update={"name": "Updated Via Repo", "positions": SAMPLE_PORTFOLIO.positions[:1]}
        )
        with session_scope(factory) as session:
