...
def test_equity_value_and_delta():
    ...
    v = LegacyDemoPricingAdapter(p).value(x)
...
def test_put_delta_is_negative():
    ...
    assert LegacyDemoPricingAdapter(p).value(x).delta < 0
...
def test_bond_dv01_negative_for_long_bond():
    ...
    assert LegacyDemoPricingAdapter(p).value(x).dv01 < 0
...
def test_pay_fixed_swap_rises_when_market_rate_rises():
    ...
    base = LegacyDemoPricingAdapter(p).value(x).market_value
    shocked = LegacyDemoPricingAdapter(p).value(x.model_copy(update={"market_swap_rate": .05})).market_value
    assert shocked > base
...
def test_ir_future_long_loses_when_forward_rises():
    ...
    base = LegacyDemoPricingAdapter(p).value(x)
    assert base.market_value == 0.0
    assert base.dv01 == -250.0
    higher = LegacyDemoPricingAdapter(p).value(x.model_copy(update={"forward_rate": 0.05}))
