...
def test_hierarchy_values_each_position_once_not_once_per_node():
    ...
    svc = PortfolioService(pricing, HistoricalRiskEngine(seed=1, observations=20), market_data=FixedMarketProvider(_two_trade_market()))
...
def test_hierarchy_parent_additives_equal_sum_of_trade_valuations():
    ...
    expected = TradeCalculationArtifact.from_valuation(pricing.value(pf.positions[0], market)).add(TradeCalculationArtifact.from_valuation(pricing.value(pf.positions[1], market)), trade_id='node')
    root = PortfolioService(BuiltinPricingEngine(), HistoricalRiskEngine(seed=1, observations=20), market_data=FixedMarketProvider(_two_trade_market())).hierarchy(pf)
...
def test_hierarchy_artifact_path_var_es_equals_summed_trade_vectors():
    ...
    svc = PortfolioService(pricing, engine, market_data=FixedMarketProvider(_two_trade_market()))
    ...
    for position in pf.positions:
        ...
        series.append(approximate_pnl_series(delta=valuation.delta, gamma=valuation.gamma, vega=valuation.vega, dv01=valuation.dv01, fx_delta=valuation.fx_delta, equity_ret=obs.equity_returns, vol_pct=obs.vol_moves, rates_bps=obs.rate_moves_bps, fx_ret=obs.fx_returns, methodology=engine.methodology))
