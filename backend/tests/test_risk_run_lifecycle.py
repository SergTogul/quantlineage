...
def _seed_portfolio(session) -> None:
    SqlAlchemyPortfolioRepository(session).save(Portfolio(id='p-life', name='Lifecycle Book', positions=[EquityPosition(type='equity', id='eq-1', symbol='AAPL', quantity=1)]))

@pytest.mark.parametrize('current,target,ok', [(RiskRunStatus.QUEUED, RiskRunStatus.RUNNING, True), (RiskRunStatus.RUNNING, RiskRunStatus.COMPLETED, True), (RiskRunStatus.RUNNING, RiskRunStatus.FAILED, True), (RiskRunStatus.QUEUED, RiskRunStatus.COMPLETED, False), (RiskRunStatus.QUEUED, RiskRunStatus.FAILED, False), (RiskRunStatus.QUEUED, RiskRunStatus.QUEUED, False), (RiskRunStatus.RUNNING, RiskRunStatus.QUEUED, False), (RiskRunStatus.RUNNING, RiskRunStatus.RUNNING, False), (RiskRunStatus.COMPLETED, RiskRunStatus.RUNNING, False), (RiskRunStatus.COMPLETED, RiskRunStatus.FAILED, False), (RiskRunStatus.COMPLETED, RiskRunStatus.QUEUED, False), (RiskRunStatus.FAILED, RiskRunStatus.RUNNING, False), (RiskRunStatus.FAILED, RiskRunStatus.QUEUED, False), (RiskRunStatus.FAILED, RiskRunStatus.COMPLETED, False)])
...
def test_assert_transition_allowed_raises():
    with pytest.raises(InvalidRiskRunTransition, match='QUEUED → COMPLETED'):
        assert_transition_allowed(RiskRunStatus.QUEUED, RiskRunStatus.COMPLETED, run_id='r1')
...
def test_elapsed_seconds_helpers():
    ...
    running = RiskRun(id='r', portfolio_id='p', status=RiskRunStatus.RUNNING, started_at=started)
    assert elapsed_seconds(running, now=started + timedelta(seconds=10)) == 10.0
    done = RiskRun(id='c', portfolio_id='p', status=RiskRunStatus.COMPLETED, started_at=started, completed_at=finished)
...
def test_happy_path_queued_running_completed(session_factory):
    with session_scope(session_factory) as session:
        ...
        queued = svc.enqueue(run_id='run-ok', portfolio_id='p-life', run_type='summary', request={'methodology': 'delta_gamma'})
...
@pytest.mark.parametrize('setup,illegal_call', [('queued', lambda svc: svc.complete('run-bad')), ('queued', lambda svc: svc.fail('run-bad', 'nope')), ('running', lambda svc: svc.start('run-bad')), ('completed', lambda svc: svc.start('run-bad')), ('completed', lambda svc: svc.fail('run-bad', 'late')), ('failed', lambda svc: svc.start('run-bad')), ('failed', lambda svc: svc.complete('run-bad'))])
...
def test_complete_with_result_rejects_when_not_running(session_factory):
    with session_scope(session_factory) as session:
        ...
        with pytest.raises(InvalidRiskRunTransition):
            svc.complete('run-early', result_type='summary', payload={'x': 1})
