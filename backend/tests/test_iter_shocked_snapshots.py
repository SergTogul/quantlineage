...
def _dataset(equity_returns: np.ndarray) -> ArrayHistoricalDataset:
    ...
    return ArrayHistoricalDataset(
        FactorObservationSeries(
            equity_returns=np.asarray(equity_returns, dtype=float),
            vol_moves=z,
            rate_moves_bps=z,
            fx_returns=z,
        )
...
def test_streaming_pnl_series_equals_list_materialized_series():
    ...
    dataset = _dataset(np.array([-0.10, 0.0, 0.05]))
    base_pv = pricing.value(book.positions[0], market).market_value

    list_pnl = np.array(
        [
            pricing.value(book.positions[0], snap).market_value - base_pv
            for snap in historical_shocked_snapshots(market, dataset)
        ],
        dtype=float,
    )
    stream_pnl = np.array(
        [
            pricing.value(book.positions[0], snap).market_value - base_pv
            for snap in iter_historical_shocked_snapshots(market, dataset)
        ],
        dtype=float,
    )
...
def test_full_reval_production_loops_use_iterator_not_list():
    ...
    assert (
        re.search(r"(?<![\w.])historical_shocked_snapshots\b", hist_src) is None
    )

    var_src = inspect.getsource(VaRAnalytics._full_reval_position_pnls)
    assert "iter_historical_shocked_snapshots" in var_src
    assert (
        re.search(r"(?<![\w.])historical_shocked_snapshots\b", var_src) is None
    )
...
def test_require_explicit_market_unchanged_on_full_reval_paths():
    ...
    pricing = BuiltinPricingEngine()
    dataset = _dataset(np.array([-0.10, 0.0]))
    with pytest.raises(ValueError, match="explicit MarketSnapshot"):
        HistoricalRiskEngine(dataset=dataset).calculate(
            book, pricing, methodology=VaRMethodology.FULL_REVALUATION
        )
    with pytest.raises(ValueError, match="explicit MarketSnapshot"):
        HistoricalRiskEngine(dataset=dataset).calculate(
            book, pricing, methodology=VaRMethodology.FULL_REVALUATION, market=None
        )
    with pytest.raises(ValueError, match="explicit MarketSnapshot"):
        VaRAnalytics(dataset=dataset).report(
            book, pricing, methodology=VaRMethodology.FULL_REVALUATION
        )
    with pytest.raises(ValueError, match="explicit MarketSnapshot"):
        VaRAnalytics(dataset=dataset).report(
