...
def tiny_portfolio() -> Portfolio:
    return Portfolio(id='err-model-book', name='Error Model Book', positions=[EquityPosition(type='equity', id='eq-1', symbol='AAPL', quantity=10)])
...
def test_body_from_http_exception_string_detail() -> None:
    body = body_from_http_exception(HTTPException(status_code=404, detail='risk run not found: x'))
    assert body == ErrorBody(code='not_found', message='risk run not found: x', details=None)

def test_body_from_http_exception_structured_detail() -> None:
    body = body_from_http_exception(HTTPException(status_code=400, detail={'code': 'unsupported_run_type', 'message': 'unsupported run_type: foo', 'details': {'run_type': 'foo'}}))
...
def test_m75_400_what_if_shape_legacy_and_v1(api_client: TestClient) -> None:
    ...
    payload = {'portfolio': portfolio, 'changes': [{'operation': 'remove', 'position_id': 'does-not-exist'}]}
...
def test_m75_422_validation_shape_legacy_and_v1(api_client: TestClient, tiny_portfolio: Portfolio) -> None:
    payload = {'portfolio': tiny_portfolio.model_dump(mode='json'), 'run_type': ''}
...
def test_m75_unsupported_run_type_400_message(api_client: TestClient, tiny_portfolio: Portfolio) -> None:
    resp = api_client.post('/risk/runs', json={'portfolio': tiny_portfolio.model_dump(mode='json'), 'run_type': 'not-a-real-type'})
    assert resp.status_code == 400
    err = resp.json()
    _assert_error_shape(err)
    assert err['code'] == 'bad_request'
