...
_QL_DIRECT = re.compile('\\bQuantLib\\b|\\bql\\.Settings\\b')
_PROCESS_LOCK = re.compile('_QL_PROCESS_LOCK|\\._session\\b|_session\\(')
_QL_IN_NATIVE = re.compile('quantlib|<ql/|ql\\.hpp|QuantLib::', re.IGNORECASE)
...
def unlocked_ql_thread_pool_violations(source: str, *, filename: str='<mem>') -> list[str]:
...
def test_detector_flags_thread_pool_that_prices_quantlib_without_lock():
    ...
    assert hits, 'detector must fail a ThreadPoolExecutor that mutates ql.Settings without _QL_PROCESS_LOCK'
...
def test_app_has_no_unlocked_quantlib_thread_pool():
    ...
    for path in _iter_app_python_files():
        violations.extend(unlocked_ql_thread_pool_violations(path.read_text(encoding='utf-8'), filename=str(path.relative_to(BACKEND_ROOT))))
    assert violations == []
...
def test_sibling_processes_isolate_quantlib_settings():
    ...
    procs = [subprocess.Popen([sys.executable, '-c', child, iso], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, env=env) for iso in dates]
...
def test_thread_pool_value_calls_remain_serialized_by_process_lock():
    ...
    bond = BondPosition(type='bond', id='zc', issuer='UST', face_value=1000000, quantity=1, maturity_years=10.0, duration=8.0)
    observed: list = []
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        futures = [pool.submit(price, '2018-01-01'), pool.submit(price, '2024-06-14')]
        for future in futures:
            future.result(timeout=30)
    assert len(observed) == 2
    expected = {engine._ql_date(date(2018, 1, 1)), engine._ql_date(date(2024, 6, 14))}
