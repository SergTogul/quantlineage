...
def _assert_reconciles(contributions, portfolio_es: float, *, abs_tol: float = 1e-6) -> None:
    total = sum(c.component_es for c in contributions)
    assert math.isclose(total, portfolio_es, rel_tol=1e-8, abs_tol=abs_tol), (
        f"contribution sum {total} vs portfolio ES {portfolio_es}"
    )
...
def test_book_desk_strategy_es_contributions_reconcile():
    ...
    # Book rollup equals sum of positions in that book
    cash_pos = sum(
        c.component_es
        for c in report.by_position
        if c.key in {"eq-a", "eq-b"}
    )
    cash_book = next(c.component_es for c in report.by_book if c.key == "Equity Cash")
    assert math.isclose(cash_pos, cash_book, abs_tol=1e-9)
...
def test_full_revaluation_position_and_factor_es_reconcile():
    ...
    _assert_reconciles(report.by_position, report.portfolio_es)
    _assert_reconciles(report.by_book, report.portfolio_es)
    # Factor path includes residual so contributions still sum to portfolio ES
    _assert_reconciles(report.by_risk_factor, report.portfolio_es)
    assert abs(report.reconciliation_error_risk_factor) < 1e-6
...
def test_var_report_includes_position_es_contributions():
    ...
    assert math.isclose(es_sum, hist.expected_shortfall, rel_tol=1e-8, abs_tol=1e-6)
...
def test_es_api_endpoint_default_methodology():
    ...
    assert math.isclose(
        sum(c["component_es"] for c in payload["by_position"]),
        payload["portfolio_es"],
        rel_tol=1e-8,
        abs_tol=1e-6,
    )
