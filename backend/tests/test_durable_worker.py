...
def tiny_portfolio() -> Portfolio:
    return Portfolio(id='worker-book', name='Worker Book', positions=[EquityPosition(type='equity', id='eq-1', symbol='AAPL', quantity=10)])
...
def test_list_by_status_memory_fifo():
    ...
    svc_portfolio = PortfolioService(create_pricing_engine(), HistoricalRiskEngine(), market_data=FixedMarketProvider(equity_spot_market('AAPL', 190.0)))
...
def test_poll_once_respects_batch_limit_and_leaves_excess_queued(tiny_portfolio):
    ...
    svc = PortfolioService(create_pricing_engine(), HistoricalRiskEngine(), market_data=FixedMarketProvider(equity_spot_market('AAPL', 190.0)))
    worker = RiskRunWorker(svc, repo=repo, max_workers=1)
    accepted = [worker.submit(portfolio=tiny_portfolio.model_copy(update={'id': f'batch-book-{idx}'}), run_type='summary', execute=False) for idx in range(3)]
...
def test_claim_queued_sqlite_marks_running(tmp_path, tiny_portfolio):
    ...
    with session_scope(factory) as session:
        SqlAlchemyPortfolioRepository(session).save(tiny_portfolio)
        SqlAlchemyRiskRunRepository(session).create(RiskRun(id='sq-1', portfolio_id=tiny_portfolio.id, status=RiskRunStatus.QUEUED, run_type='summary'))
    with session_scope(factory) as session:
