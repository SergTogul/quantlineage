...
def _eq_m(spot, *, symbol="ABC", rate=0.04, vol=None, div=0.0):
    kw = {"id": "t", "equity_spots": {symbol: spot}, "rates": {"USD": rate}, "dividend_yields": {symbol: div}}
    if vol is not None:
        kw["equity_vols"] = {symbol: vol}
    return MarketSnapshot(**kw)

def _usd_m(rate, *, proj=None, vol=None, quote=None):
...
LOCAL_EQUITY = MarketSnapshot(id="local-eq", equity_spots={"AUTH": 999.0}, rates={"USD": 0.04})
LOCAL_FUTURE = MarketSnapshot(
    id="local-fut",
    equity_spots={"AUTH": 999.0},
    rates={"USD": 0.04},
    dividend_yields={"AUTH": 0.0},
)
LOCAL_OPTION = MarketSnapshot(
    id="local-opt",
    equity_spots={"AUTH": 999.0},
    equity_vols={"AUTH": 0.20},
    rates={"USD": 0.03},
    dividend_yields={"AUTH": 0.0},
)

def _local_for(position) -> MarketSnapshot:
    t = getattr(position, "type", None)
    if t == "equity":
        return LOCAL_EQUITY
    if t == "equity_future":
        return LOCAL_FUTURE
...
def test_supplied_snapshot_beats_trade_local_equity_spot() -> None:
    ...
    assert valuation.market_value != position.quantity * 999.0


def test_explicit_local_style_snapshot_prices_former_dto_marks() -> None:
    _, position = _equity_book()

    valuation = BuiltinPricingEngine().value(position, LOCAL_EQUITY)
...
def test_supplied_snapshot_beats_trade_local_equity_future_spot() -> None:
    ...
    valuation = BuiltinPricingEngine().value(position, market)
    local = BuiltinPricingEngine().value(position, _local_for(position))

    assert valuation.market_value != local.market_value
    assert valuation.market_value != position.quantity * position.multiplier * 999.0
    assert position.model_dump(mode="json") == original
...
def test_quantlib_supplied_snapshot_beats_trade_local_equity_future_spot() -> None:
    ...
    local = engine.value(position, _local_for(position))
...
def test_equity_option_vol_comes_from_explicit_snapshot() -> None:
    _, position = _option_book()
    original = position.model_dump(mode="json")

    valuation = BuiltinPricingEngine().value(position, LOCAL_OPTION)
    richer = BuiltinPricingEngine().value(
        position,
        LOCAL_OPTION.model_copy(update={"equity_vols": {"AUTH": 0.40}}),
    )
...
def test_supplied_snapshot_beats_trade_local_equity_future_rate_and_div() -> None:
    ...
    original = position.model_dump(mode="json")
    pricing = BuiltinPricingEngine()
    local = pricing.value(position, _local_for(position))
...
def test_equity_family_rate_and_div_come_from_explicit_snapshot(
    book_factory,
) -> None:
    _, position = book_factory()
    original = position.model_dump(mode="json")
    local = _local_for(position)

    valuation = BuiltinPricingEngine().value(position, local)
    richer_rate = BuiltinPricingEngine().value(
        position,
        local.model_copy(update={"rates": {"USD": 0.08}}),
    )
    richer_div = BuiltinPricingEngine().value(
        position,
        local.model_copy(update={"dividend_yields": {"AUTH": 0.05}}),
    )
