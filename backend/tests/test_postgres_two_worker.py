...
def require_live_postgres() -> str:
    ...
    * ``CI`` set and DSN missing: skip (PR ``backend-pytest`` has no Postgres).
    ...
    if nightly or (ci and url):
        ...
        pytest.fail(
            "RISKFORGE_DATABASE_URL must be a reachable postgresql DSN when "
            f"CI or RISKFORGE_NIGHTLY is set ({detail})"
        )
    pytest.skip(
        "live Postgres required (nightly GHA service; local docker optional — "
        "see BUILD_NOTES / .github/workflows/nightly.yml)"
    )
...
def _seed_book_and_runs(factory, *, n_runs: int) -> tuple[str, list[str]]:
    ...
    book = Portfolio(
        id=book_id,
        name="Nightly two-worker book",
        positions=[
            EquityPosition(type="equity", id=f"{book_id}-eq", symbol="AAPL", quantity=1, price=10.0)
        ],
    )
...
def test_two_workers_do_not_double_claim_one_queued_run():
    ...
    def _worker() -> None:
        ...
        except BaseException as exc:  # noqa: BLE001 — surface both threads
            errors.append(exc)
