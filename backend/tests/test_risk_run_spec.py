...
def _full_spec_run() -> RiskRun:
    return RiskRun(id='run-spec', portfolio_id='p-spec', market_snapshot_id='snap-spec', created_at=_ts(), pricing_engine_version='builtin-1.0', methodology=VaRMethodology.DELTA_GAMMA, scenario_set=['base', 'eq-crash'], historical_dataset_id='demo-historical-factors', historical_dataset_version='v1', as_of=date(2026, 9, 2), calculation_config=RiskRunCalculationConfig(observations=750, seed=7, confidence=0.99))
...
def test_mapping_round_trip_preserves_spec_fields():
    ...
    assert row.calculation_config == {'observations': 750, 'seed': 7, 'confidence': 0.99}
    back = row_to_risk_run(row)
    assert back.model_dump() == run.model_dump()

def test_mapping_old_row_without_spec_fields_loads():
    row = RiskRunRow(id='run-legacy', portfolio_id='p-legacy', status=RiskRunStatus.QUEUED, run_type='summary', request={}, scenario_set=[], created_at=_ts())
...
def test_sqlite_persist_reload_equals_full_spec():
    portfolio = Portfolio(id='p-spec', name='Spec Book', positions=[EquityPosition(type='equity', id='eq-1', symbol='AAPL', quantity=1)])
...
def test_sqlite_old_run_without_spec_fields_still_loads():
    portfolio = Portfolio(id='p-old', name='Old Book', positions=[EquityPosition(type='equity', id='eq-1', symbol='AAPL', quantity=1)])
    factory = make_sqlite_session_factory()
    with session_scope(factory) as session:
        SqlAlchemyPortfolioRepository(session).save(portfolio)
        SqlAlchemyRiskRunRepository(session).create(RiskRun(id='run-old', portfolio_id='p-old'))
    with session_scope(factory) as session:
...
def _tiny_book() -> Portfolio:
    return Portfolio(id='p-enqueue', name='Enqueue Book', positions=[EquityPosition(type='equity', id='eq-1', symbol='AAPL', quantity=1)])

def test_enqueue_persists_explicit_spec_fields():
    repo = InMemoryRiskRunRepository()
    svc = RiskRunService(repo)
    run = svc.enqueue(run_id='enq-spec', portfolio_id='p-enqueue', historical_dataset_id='demo-historical-factors', historical_dataset_version='v1', as_of=date(2026, 9, 2), calculation_config=RiskRunCalculationConfig(observations=750, seed=7, confidence=0.99))
    assert run.historical_dataset_id == 'demo-historical-factors'
    assert run.historical_dataset_version == 'v1'
    assert run.as_of == date(2026, 9, 2)
    assert run.calculation_config is not None
...
def test_submit_with_request_spec_persists_columns():
    ...
    view = worker.submit(portfolio=_tiny_book(), run_type='summary', execute=False, request={'methodology': 'DELTA_GAMMA', 'historical_dataset_id': 'demo-historical-factors', 'historical_dataset_version': 'v1', 'as_of': '2026-09-02', 'calculation_config': {'observations': 750, 'seed': 7, 'confidence': 0.99}})
...
def test_submit_parses_as_of_from_request(as_of: str):
    repo = InMemoryRiskRunRepository()
    worker = RiskRunWorker(build_portfolio_service(), repo=repo, max_workers=1)
    view = worker.submit(portfolio=_tiny_book(), execute=False, request={'as_of': as_of})
    worker.shutdown(wait=False)
    stored = repo.get(view.id)
    assert stored is not None
    if as_of in {'current', 't0'}:
