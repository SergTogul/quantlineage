...
Set ``QUANTLINEAGE_REQUIRE_QUANTLIB=1`` so a missing QuantLib install fails the
job instead of ``pytest.importorskip`` skipping to green (R0.1.6 / RF-016).

``require_quantlib_for_nightly`` fail-closes when ``QUANTLINEAGE_NIGHTLY=1`` so
...
def require_quantlib_for_nightly(*, _importer=None):
    ...
    * ``QUANTLINEAGE_NIGHTLY`` set: **fail** if QuantLib cannot be imported
      (nightly QuantLib E2E; no skip-green).
    * ``QUANTLINEAGE_REQUIRE_QUANTLIB`` set: **fail** (same intent as the hard-gate).
    * Otherwise: skip if missing (local optional).
    ...
    try:
        ...
        if _env_flag("QUANTLINEAGE_NIGHTLY") or quantlib_required():
            pytest.fail(
                "QuantLib must be importable when QUANTLINEAGE_NIGHTLY=1 or "
                "QUANTLINEAGE_REQUIRE_QUANTLIB=1 (no skip-green)"
            )
