...
def _assert_reconciles(report, *, abs_tol: float = 1e-6) -> None:
    explained = sum(i.pnl for i in report.items)
    assert math.isclose(explained, report.explained_change, abs_tol=abs_tol, rel_tol=1e-8)
    assert math.isclose(
        report.explained_change + report.residual,
        report.total_change,
        abs_tol=abs_tol,
        rel_tol=1e-8,
    )
    assert math.isclose(
        report.total_change,
        report.current_market_value - report.base_market_value,
        abs_tol=abs_tol,
        rel_tol=1e-8,
    )
...
def test_identical_state_zero_attribution():
    ...
    book = Portfolio(
        ...
        name="p",
        positions=[
            EquityPosition(type="equity", id="e", symbol="SPY", quantity=10, price=100.0),
        ],
    )
...
def test_equity_spot_move_explained_by_delta():
    ...
    curr = prev.model_copy(
        update={"id": "t1", "equity_spots": {"ABC": 101.0}},
    )
    book = Portfolio(
        id="p",
        name="p",
        positions=[
            EquityPosition(type="equity", id="e", symbol="ABC", quantity=100, price=100.0),
        ],
    )
    report = engine.explain(
        AttributionRequest(
    ...
    # Cash delta × 1% return = 100 * 100 * 0.01 = 100
    assert math.isclose(drivers[DRIVER_DELTA], 100.0, abs_tol=1e-9)
    assert abs(drivers.get(DRIVER_GAMMA, 0.0)) < 1e-12
    assert abs(report.residual) < 1e-9
    assert math.isclose(report.total_change, 100.0, abs_tol=1e-9)
    _assert_reconciles(report)
...
def test_option_spot_move_has_delta_and_gamma():
    ...
    assert drivers[DRIVER_GAMMA] > 0.0  # long call gamma
    market_move = _pv(book, curr) - _pv(book, prev)
    assert math.isclose(report.total_change, market_move, abs_tol=1e-9)
    # Taylor residual small vs move for 5% bump
...
def test_vol_move_explained_by_vega():
    ...
    assert abs(drivers.get(DRIVER_DELTA, 0.0)) < 1e-9
...
def test_rate_move_explained_by_rates_bucket():
    ...
    drivers = _by_driver(report)
    # DV01 × +10bp (Builtin DV01 is duration-based; residual absorbs convexity /
    # compounding vs analytic duration — still reconciles exactly).
...
def test_fx_move_explained_by_fx_bucket():
    ...
    curr = prev.model_copy(
        update={"id": "t1", "fx_spots": {"EURUSD": 1.111}},
    )
    book = Portfolio(
        id="p",
        name="p",
...
def test_new_and_closed_trades():
    ...
    prev = Portfolio(
        ...
        name="p",
        positions=[
            EquityPosition(type="equity", id="keep", symbol="A", quantity=10, price=50.0),
            EquityPosition(type="equity", id="gone", symbol="B", quantity=5, price=80.0),
        ],
    )
    curr = Portfolio(
        id="p",
        ...
        positions=[
            EquityPosition(type="equity", id="keep", symbol="A", quantity=10, price=50.0),
            EquityPosition(type="equity", id="fresh", symbol="B", quantity=2, price=80.0),
        ],
    )
    report = engine.explain(
        AttributionRequest(
    ...
    drivers = _by_driver(report)
    assert math.isclose(drivers[DRIVER_CLOSED], -5 * 80.0, abs_tol=1e-9)
    assert math.isclose(drivers[DRIVER_NEW], 2 * 80.0, abs_tol=1e-9)
    assert abs(report.residual) < 1e-9
    _assert_reconciles(report)
...
def test_quantity_increase_is_new_trades():
    ...
    prev = Portfolio(
        id="p",
        name="p",
        positions=[EquityPosition(type="equity", id="e", symbol="A", quantity=10, price=40.0)],
    )
    curr = Portfolio(
        id="p",
        name="p",
        positions=[EquityPosition(type="equity", id="e", symbol="A", quantity=15, price=40.0)],
    )
    report = engine.explain(
        AttributionRequest(
            previous_portfolio=prev,
    ...
    drivers = _by_driver(report)
    assert math.isclose(drivers[DRIVER_NEW], 5 * 40.0, abs_tol=1e-9)
...
def test_sample_demo_reconciles():
    ...
    _assert_reconciles(report, abs_tol=1e-6)
    # Explained + residual identity already checked; residual is Taylor remainder
