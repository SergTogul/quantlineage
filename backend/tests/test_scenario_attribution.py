...
def _stress_market() -> MarketSnapshot:
    return equity_spots_market(
        {"SPY": 500.0, "NVDA": 120.0, "QQQ": 400.0},
        vols={"SPY": 0.2, "NVDA": 0.4, "QQQ": 0.22},
    )

def _empty_market() -> MarketSnapshot:
    return MarketSnapshot(id="empty", rates={"USD": 0.04})
...
def _multi_desk_portfolio() -> Portfolio:
    ...
    return Portfolio(id='multi-desk-stress', name='Multi Desk Stress', firm='Acme Capital', desk='Default Desk', strategy='Default Strat', positions=[EquityPosition(type='equity', id='eq-a', symbol='SPY', quantity=100, book='Cash A', desk='Rates Desk', strategy='Carry'), EquityPosition(type='equity', id='eq-b', symbol='NVDA', quantity=50, book='Cash B', desk='Equity Desk', strategy='Momentum'), EquityPosition(type='equity', id='eq-c', symbol='QQQ', quantity=25, book='Cash B')])
...
def test_multi_desk_strategy_placement_attribution():
    ...
    _assert_reconciles(report.by_desk, report.portfolio_pnl)
    _assert_reconciles(report.by_strategy, report.portfolio_pnl)
    assert report.reconciliation_error_desk < 1e-09
    assert report.reconciliation_error_strategy < 1e-09
    ...
    assert math.isclose(desk_by_key['Rates Desk'], trade_by_id['eq-a'], abs_tol=1e-09)
    assert math.isclose(desk_by_key['Equity Desk'], trade_by_id['eq-b'], abs_tol=1e-09)
    assert math.isclose(desk_by_key['Default Desk'], trade_by_id['eq-c'], abs_tol=1e-09)
    assert math.isclose(strategy_by_key['Carry'], trade_by_id['eq-a'], abs_tol=1e-09)
    assert math.isclose(strategy_by_key['Momentum'], trade_by_id['eq-b'], abs_tol=1e-09)
    assert math.isclose(strategy_by_key['Default Strat'], trade_by_id['eq-c'], abs_tol=1e-09)

def test_risk_factor_contributions_reconcile_with_interaction():
    pricing = BuiltinPricingEngine()
    scenario = StressScenario(id='multi', name='Eq+Vol+Rates', kind=ScenarioKind.MACRO, equity_shock=-0.15, vol_shock=0.4, rates_shift_bps=100)
    report = ScenarioAttributionEngine().decompose(SAMPLE_PORTFOLIO, pricing, scenario, market=SAMPLE_MARKET)
    assert report.by_risk_factor
    keys = {c.key for c in report.by_risk_factor}
    assert 'interaction' in keys
    assert keys & {'SPY', 'NVDA'}
    _assert_reconciles(report.by_risk_factor, report.portfolio_pnl)
    assert report.reconciliation_error_risk_factor < 1e-06

def test_formal_scenario_decomposition():
    pricing = BuiltinPricingEngine()
    formal = Scenario(id='typed_eq_vol', name='Typed equity+vol', category=ScenarioCategory.MACRO, description='Formal Scenario path', shocks=(FactorShock(EquitySpot('SPY'), -0.1), FactorShock(EquityVol(underlying='SPY'), 0.25), FactorShock(RateZero(currency='USD', tenor='ALL'), 0.01)), threshold=ScenarioThreshold(max_loss_pct=0.1))
    report = ScenarioAttributionEngine().decompose(SAMPLE_PORTFOLIO, pricing, formal, market=SAMPLE_MARKET)
...
def test_empty_portfolio_yields_empty_contributions():
    ...
    report = ScenarioAttributionEngine().decompose(empty, pricing, scenario, market=_empty_market())
    assert report.portfolio_pnl == 0.0
...
def test_stress_engine_evaluate_includes_contributions():
    pricing = BuiltinPricingEngine()
    report = StressEngine().evaluate(SAMPLE_PORTFOLIO, pricing, DEFAULT_SCENARIOS[:2], market=SAMPLE_MARKET)
    for ev in report.evaluations:
        assert ev.contributions is not None
        assert math.isclose(ev.contributions.portfolio_pnl, ev.pnl, abs_tol=1e-09)
...
def test_deterministic_decomposition():
    ...
    a = ScenarioAttributionEngine().decompose(SAMPLE_PORTFOLIO, pricing, scenario, market=SAMPLE_MARKET)
    b = ScenarioAttributionEngine().decompose(SAMPLE_PORTFOLIO, pricing, scenario, market=SAMPLE_MARKET)
