...
def _skewed_equity_surface(name: str='ABC', atm: float=0.2, skew: float=0.5) -> VolSurface:
...
def test_option_vol_fallback_without_surface():
    ...
    vol = option_vol_from_snapshot(market, name='ABC', maturity_years=1.0, strike=110.0, spot=100.0, fallback=0.22)
...
def test_builtin_equity_option_pv_differs_on_skewed_surface():
    ...
    opt = EuropeanOptionPosition(type='european_option', id='o', symbol='ABC', quantity=1, strike=110.0, maturity_years=1.0, option_type='call')
    scalar = MarketSnapshot(equity_spots={'ABC': 100.0}, equity_vols={'ABC': 0.2}, rates={'USD': 0.03}, dividend_yields={'ABC': 0.0})
    skewed = _skewed_equity_surface('ABC', atm=0.2, skew=0.5)
    assert skewed.vol(1.0, 1.1) == pytest.approx(0.25)
    assert skewed.atm_vol() == pytest.approx(0.2)
...
def test_builtin_flat_surface_matches_scalar_atm():
    opt = EuropeanOptionPosition(type='european_option', id='o', symbol='ABC', quantity=1, strike=100.0, maturity_years=1.0, option_type='call')
    scalar = MarketSnapshot(equity_spots={'ABC': 100.0}, equity_vols={'ABC': 0.2}, rates={'USD': 0.03}, dividend_yields={'ABC': 0.0})
    flat = attach_vol_surface(scalar, build_equity_vol_surface('ABC', 0.2))
    engine = BuiltinPricingEngine()
...
def test_builtin_flat_surfaces_match_scalar_after_typed_vol_apply():
    eq_opt = EuropeanOptionPosition(type='european_option', id='eqo', symbol='ABC', quantity=1, strike=100.0, maturity_years=1.0, option_type='call')
    fx_opt = FXOptionPosition(type='fx_option', id='fxo', pair='EURUSD', notional_base=1000000, strike=1.1, maturity_years=1.0, option_type='call')
    scalar = MarketSnapshot(equity_spots={'ABC': 100.0}, equity_vols={'ABC': 0.2}, fx_spots={'EURUSD': 1.1}, fx_vols={'EURUSD': 0.1}, rates={'USD': 0.04, 'EUR': 0.03}, dividend_yields={'ABC': 0.0})
    with_surfaces = attach_vol_surface(attach_vol_surface(scalar, build_equity_vol_surface('ABC', 0.2)), build_fx_vol_surface('EURUSD', 0.1))
    ...
    engine = BuiltinPricingEngine()
    assert engine.value(eq_opt, bumped_surfaces).market_value == pytest.approx(engine.value(eq_opt, bumped_scalar).market_value)
    assert engine.value(fx_opt, bumped_surfaces).market_value == pytest.approx(engine.value(fx_opt, bumped_scalar).market_value)

def test_builtin_fx_option_pv_differs_on_skewed_surface():
    opt = FXOptionPosition(type='fx_option', id='fxo', pair='EURUSD', notional_base=1000000, strike=1.21, maturity_years=1.0, option_type='call')
    scalar = MarketSnapshot(fx_spots={'EURUSD': 1.1}, fx_vols={'EURUSD': 0.1}, rates={'USD': 0.04, 'EUR': 0.03})
    skewed = build_fx_vol_surface('EURUSD', 0.1).skew_shock(0.5)
    with_surface = attach_vol_surface(scalar, skewed)
    engine = BuiltinPricingEngine()
    pv_scalar = engine.value(opt, scalar).market_value
    pv_surface = engine.value(opt, with_surface).market_value
...
def test_quantlib_equity_option_pv_differs_on_skewed_surface():
    opt = EuropeanOptionPosition(type='european_option', id='o', symbol='ABC', quantity=1, strike=110.0, maturity_years=1.0, option_type='call')
    scalar = MarketSnapshot(equity_spots={'ABC': 100.0}, equity_vols={'ABC': 0.2}, rates={'USD': 0.03}, dividend_yields={'ABC': 0.0})
    with_surface = attach_vol_surface(scalar, _skewed_equity_surface('ABC', 0.2, 0.5))
