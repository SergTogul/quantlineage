...
Override with ``RISKFORGE_DATABASE_URL``. CI and unit tests use SQLite
...
DEFAULT_POSTGRES_DSN = "postgresql+psycopg://riskforge:riskforge@localhost:5432/riskforge"
...
def get_configured_database_url() -> str | None:
    """Return ``RISKFORGE_DATABASE_URL`` when explicitly set (non-blank).
    ...
    """
    raw = os.environ.get("RISKFORGE_DATABASE_URL")
    if raw is None:
        return None
    stripped = raw.strip()
...
def get_database_settings(
    *,
    ...
    Env vars:
    - ``RISKFORGE_DATABASE_URL`` — SQLAlchemy URL (Postgres or SQLite)
    - ``RISKFORGE_DB_ECHO`` — ``1`` / ``true`` to log SQL
    """
    resolved = url or get_configured_database_url() or DEFAULT_SQLITE_DSN
    if echo is None:
        raw = os.environ.get("RISKFORGE_DB_ECHO", "").strip().lower()
