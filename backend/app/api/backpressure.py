...
(``QUANTLINEAGE_EXTERNAL_WORKER=1``) or explicitly disables inline heavy
compute (``QUANTLINEAGE_HEAVY_INLINE=0``), HEAVY handlers refuse
...
ENV_HEAVY_INLINE = "QUANTLINEAGE_HEAVY_INLINE"
...
def heavy_inline_allowed() -> bool:
    ...
    Refuse when ``QUANTLINEAGE_HEAVY_INLINE`` is an explicit falsey value, or
    when ``QUANTLINEAGE_EXTERNAL_WORKER=1`` (Compose backend + worker process).
