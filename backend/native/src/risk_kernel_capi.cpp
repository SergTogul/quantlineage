...
extern "C" int riskforge_kernel_abi_version(void) {
  return RISKFORGE_KERNEL_ABI;
}

extern "C" int riskforge_portfolio_scenarios(
    int abi,
    ...
    double* out,
    std::size_t n_out) {
  if (abi != RISKFORGE_KERNEL_ABI) {
    return RISKFORGE_KERNEL_ERR_ABI;
  }

  constexpr std::size_t kMax = std::numeric_limits<std::size_t>::max();
  if (n_exposures > kMax / RISKFORGE_KERNEL_EXPOSURE_STRIDE ||
      n_shocks > kMax / RISKFORGE_KERNEL_SHOCK_STRIDE) {
    return RISKFORGE_KERNEL_ERR_LENGTH;
  }
  const std::size_t exp_need = n_exposures * static_cast<std::size_t>(RISKFORGE_KERNEL_EXPOSURE_STRIDE);
  const std::size_t shk_need = n_shocks * static_cast<std::size_t>(RISKFORGE_KERNEL_SHOCK_STRIDE);
  if (n_exposure_doubles != exp_need || n_shock_doubles != shk_need || n_out != n_shocks) {
    return RISKFORGE_KERNEL_ERR_LENGTH;
  }

  if (n_exposures > 0 && exposures == nullptr) {
    return RISKFORGE_KERNEL_ERR_NULL;
  }
