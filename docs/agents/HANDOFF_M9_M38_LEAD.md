# Handoff — M9.8 disposition + M3.8 formal Scenario wire

## Task
Lead Architect: close Milestone 9 honestly (M9.8 Redis/RQ disposition); implement M3.8 formal Scenario HTTP wire

## Owner
Lead Architect / Orchestrator (implemented Backend/API + ROADMAP; DevOps consulted for M9.8 — no Redis/RQ code)

## Summary
- **M9 remaining open (pre-disposition):** only M9.8 (Redis/RQ optional). M9.1–M9.7 + M9.9–M9.10 already DONE.
- **M9.8 decision:** ROADMAP treated Redis/RQ as **optional**. Compose already ships `postgres` + `backend` + `worker` + `frontend` with Postgres `SKIP LOCKED` claim (M5.7 / ADR 005). **Did not** add Redis/RQ (would be ceremony without product semantics; must not regress claim path). Marked **M9.8 containers DONE**; Redis/RQ **DEFERRED** accepted residual (not fake Redis `[x]`). **Milestone 9 COMPLETE**.
- **M3.8:** formal `ScenarioWire` HTTP under `/api/v1/risk/stress/formal/*` + `GET .../scenarios/formal`; adapters project to legacy `StressScenario` for `StressEngine`. Legacy endpoints unchanged. PricingEngine / VaR math / SKIP LOCKED untouched.

## Files changed
- `ROADMAP.md` — M9 COMPLETE + M9.8 disposition; M3.8 DONE
- `docs/adr/004-formal-scenario-domain-model.md` — M3.8 consequences
...
- `backend/app/risk/scenario_model.py` — docstring (M3.8 pointer)
- `backend/tests/test_scenario_wire_api.py` — adapter + API parity tests (new)
- `docs/agents/HANDOFF_M9_M38_LEAD.md` (this file)
...
## Results
- Backend: focused M3.8 + durable worker **15 passed**; stress/API suite **72 passed** (1 Starlette/httpx deprecation warning)
- Frontend: unchanged
...
- Owner: Frontend (optional) — ScenarioBuilder emit formal wire; or Stress — M3.9 methodology docs; or C++ — M6 risk-path SLA; or Demo — M10
- Requested action: pick next highest-value residual (prefer M3.9 docs, M6 SLA evidence, or M10 demo data)
- Blocking?: no
