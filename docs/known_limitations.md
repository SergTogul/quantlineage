...
- Public-data mode currently covers US equity spot and USD Treasury-rate factors (AAPL, MSFT, NVDA, SPY + FRED DGS2/DGS5/DGS10). FX and volatility remain outside the public-data Wave A universe. Frozen history is stored as `data/public_history/real-public-wave-a/<content-hash>.csv`; public snapshot ids are `real:public:wave-a:{as_of}:{content_hash}`. This is not the entire existing cross-asset derivatives book running from public data.
