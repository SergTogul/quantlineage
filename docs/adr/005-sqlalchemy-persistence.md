...
- Status: Accepted (M5.1 schema + repos; M5.6 FastAPI DI **DONE**; M5.9 stress HTTP ← scenario_definitions DI **DONE**; M5.7 Compose worker + Postgres `SKIP LOCKED` claim **DONE**; Postgres CI smoke authored)
- Date: 2026-09-02
...
7. Risk-run status enum lives in domain ``RiskRun`` / ``RiskRunStatus``
   ...
   same enum. Lifecycle service is M5.3; async HTTP APIs are M5.4.
...
|-------------|-------------------------|
| ORM inside ``app/risk/`` | Couples M4 risk agents to DB churn; charter prefers persistence boundary. |
...
- FastAPI DI (M5.6 **DONE**): when ``RISKFORGE_DATABASE_URL`` is set, lifespan
...
- Stress HTTP (M5.9 **DONE**): ``GET /risk/stress/scenarios`` and default
...
- Alembic initial revision: ``001_initial_persistence``; M5.2 append-only
...
- **M5.7 (DONE):** Compose ``worker`` runs ``python -m app.worker`` (same
...
- Local / CI Postgres smoke: ``./scripts/smoke_postgres.sh`` (requires
  ``RISKFORGE_DATABASE_URL``). Runner validation remains **M9.9**.
