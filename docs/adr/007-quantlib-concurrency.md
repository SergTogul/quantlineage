...
- Status: Accepted (codifies existing adapter + worker design; closes ROADMAP M6.6)
- Date: 2026-09-02
...
Separately, Milestone 6 ships an optional C++ scenario aggregation kernel
...
- Worker docstring: thread pool, not a distributed queue; Compose “worker” is a
  separate **process** polling Postgres (M5.7), not threaded QL reval.
- Native README / M6.3: FULL_REVALUATION stays on `PricingEngine`; native kernel
...
| Move QuantLib into the native scenario `.so` | Violates “pricing library prices; RiskForge aggregates”; couples M6 ABI to QL. |
| ProcessPoolExecutor for every risk run now | Heavier ops change; M5.7 still documents single-process poll + future claim/lease. Prefer documenting the target architecture over premature rewrite. |
...
- Product speed claims for FULL_REVALUATION must cite process-level scaling or
  a future native pricing service — not M6 scenario-kernel microbenchmarks.
