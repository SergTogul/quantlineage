...
(`equity_shock`, `equity_shocks`, …). Historical VaR introduced
...
Workstream 3 needs a formal scenario definition: identity, category, typed
...
- + multi-factor engine and crisis library should build on ``Scenario``.
- ** (2026-09-02):** formal ``ScenarioWire`` / ``FactorShockWire`` exposed on
