# Canonical API prefix `/api/v1` and legacy path sunset **Owner:** Backend/API Engineer (with Lead Architect)
...
| Phase | When | Action |
|-------|------|--------|
| **Deprecate** (this task) | 2026-09-02 | Document `/api/v1` as canonical; dual-mount kept; Deprecation/Sunset/Link on legacy |
| **Migrate clients** | **DONE** 2026-09-02 ( Frontend) | SPA `frontend/src/api.js` uses `/api/v1`; legacy dual-mount kept for other callers |
...
2. Keep using the same JSON request/response shapes — did not fork schemas by mount.
3. Optionally assert that canonical responses lack `Deprecation` / `Sunset`.
4. After migration, ignore legacy paths; they will be removed in a later workstream.

**Frontend note:** SPA `api.js` migrated to `/api/v1` (2026-09-02, ). Legacy
